...
df %>% filter(sample=="18071_12_1") %>% 
  select("mz", "intensity") %>%
  ggplot(aes(x=mz, y=intensity)) + geom_bar(stat="identity") + ggtitle("Strike 1 Spectrometry View") + theme(axis.title=element_text(size=6),axis.text=element_text(size=6),plot.title = element_text(hjust = 0.5, size=10),axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + labs(y="Intensity", x = "m/z")
```
...
annotate_figure(arrangement, top = text_grob("PCAs showing how the batch effect changes",face = "italic", size = 15), bottom = text_grob("X axis shows PC1 and Y axis shows PC2",face = "italic", size = 12))
rm(arrangement)
```
...
arrangement<- ggarrange(Wavecut1,Wavecut0.9,Wavecut0.7,Wavecut0.6,Wavecut0.5,Wavecut0.4,Wavecut0.3,Wavecut0.2,Wavecut0.1,Wavecut0,
