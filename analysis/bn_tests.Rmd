...
## Storing the means of each batch
```{r}
meanofbatch1<-mdf %>% 
  filter(batch==1) %>% 
...
## Removing the means of each batch

```{r}
batch1<-mdf %>% filter(batch==1) %>% select(starts_with("M")) %>% -meanofbatch1

test<-mdf %>% 
  mutate(meanofbatch = ifelse(batch == "1", meanofbatch1, 
                              ifelse(batch =="2", meanofbatch2,
                                     ifelse(batch=="3",meanofbatch3,
                                            ifelse(batch=="2b", meanofbatch2b,
                                                   ifelse(batch=="b2", meanofbatchb2,
                                                          ifelse(batch=="b3", meanofbatchb2,
                                                                 ifelse(batch=="b", meanofbatch3,0))))))))

test$meanofbatch[is.na(test$meanofbatch)] <- 0

test2<-test %>% select(starts_with("M")) %>% -test$meanofbatch

mdf <- cbind(rowinfo,test2)
mdf <- mdf %>% select(-rowid,-meanofbatch)
mdf[is.na(mdf)] <- 0
```
##PCA
```{r}
