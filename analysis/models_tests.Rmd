...
```{r}
error1<-fit7$pred %>%
  filter(alpha == fit7$bestTune$alpha & lambda == fit7$bestTune$lambda) %>% #subset 
  separate(Resample, c("fold", "rep"), "\\.") %>% 
  group_by(rep) %>% #group by replicate
  summarise(rmse = RMSE(obs, pred)) %>% 
  as.data.frame() %>% 
...
```{r}
rm(error1,mdf,ms,rowinfo,training_DF,training_x,trControl,values,i,sumofrows,training_y,xvar,plot_pca,root_transform, yvar)
