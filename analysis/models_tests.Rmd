...
tunelengths[i-1,] = data.frame(getTrainPerf(A_Input_data), Name = paste0("tunelength", i-1), QC = "none")
}
...
results[1,] = data.frame(getTrainPerf(A_Input_data), Name = paste0("A_Input_data"), QC = "none")
```

\#\#Getting the RMSE of each repeat: <https://stackoverflow.com/questions/56950684/how-to-get-predictions-for-each-fold-in-10-fold-cross-validation-of-the-best-tun/56965881#56965881>

Looking at the predictions:

```{r}
head(A_Input_data$pred)
```
...
```{r}
A_Input_data$pred %>%
  filter(alpha == A_Input_data$bestTune$alpha & lambda == A_Input_data$bestTune$lambda) %>% head()
```
...
## Size of the dataframe and saving the processed mdf
