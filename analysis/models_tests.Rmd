...
One method I found of removing PCA outliers without using quantiles is using the boxplot function. <https://www.r-bloggers.com/2020/01/how-to-remove-outliers-in-r/>
...
# Model after row normalization
## Trying to use a package I found
I found this package... Does it do what we want?
...
values<-normalize(target_values, method = "range", range = c(0, 1), margin = 1L)

tmp1 <- target_info 
tmp2 <- values[tmp1$rowid,]

tmp3 <- target_info 
tmp4 <- values[tmp1$rowid,]

tmp3 <- target_info %>% filter(type %in% c("sample"))
tmp4 <- target_values[tmp3$rowid,]
plot_pca(tmp3,tmp4, color_label = "age")


#Substituting the original dataframe
rowinfo<-tmp1
values<-tmp2
mdf <- cbind(rowinfo,values)
mdf <- mdf %>% select(-rowid)
...
results[6,] = data.frame(getTrainPerf(fit6), Name = paste0("fit6"), QC = "4th_root + PCA outlier removal + Bad features removal + QC and Bind samples + Row Normalization")
