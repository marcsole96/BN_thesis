...
```{r warning=FALSE}
trControl <- trainControl(method = "repeatedcv", number = 5, verboseIter = F, savePredictions = "final")
...
fit1
fit2
predictions<-extractPrediction(list(my_rf = fit1), testX = training_x, testY = training_y)
```

```{r}
results <- resamples(list(lm=fit1, glm=fit2))
summary(results)
bwplot(results)
dotplot(results)
```
