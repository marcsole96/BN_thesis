...
```{r}
results[7,] = data.frame(getTrainPerf(J_PQN_Rpcm), Name = paste0("J_PQN_Rpcm"), Details = "PQN Rcpm")

error1<-J_PQN_Rpcm$pred %>%
  filter(alpha == J_PQN_Rpcm$bestTune$alpha & lambda == J_PQN_Rpcm$bestTune$lambda) %>% #subset 
  separate(Resample, c("fold", "rep"), "\\.") %>% 
  group_by(rep) %>% #group by replicate
  summarise(RMSE = RMSE(obs, pred)) %>% 
  as.data.frame() %>% 
  mutate(Model = "J_PQN_Rpcm", Details = "PQN Rcpm")
...
```{r}
results[2,] = data.frame(getTrainPerf(wave_0.3), Name = paste0("wave_0.3"), Details = "WaveICA2.0 Cutoff 0.3")
...
```{r}
results[3,] = data.frame(getTrainPerf(wave_0.5), Name = paste0("wave_0.5"), Details = "WaveICA2.0 Cutoff 0.5")
...
```{r}
results[4,] = data.frame(getTrainPerf(wave_0.7), Name = paste0("wave_0.7"), Details = "WaveICA2.0 Cutoff 0.7")
...
```{r}
results[5,] = data.frame(getTrainPerf(wave_1), Name = paste0("wave_1"), Details = "WaveICA2.0 Cutoff 1")
